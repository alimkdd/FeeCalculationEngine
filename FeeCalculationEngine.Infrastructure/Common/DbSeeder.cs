...
namespace FeeCalculationEngine.Infrastructure.Common;
...
public static class DbSeeder
{
    public static async Task Initialize(AppDbContext context, IHostEnvironment environment, IConfiguration configuration)
    {
        ...
        if (environment.IsDevelopment())
        {
            ...
            databaseDirectory = Path.GetFullPath(Path.Combine(solutionDirectory, "FeeCalculationEngine.Infrastructure"));
        }
        else
        {
        ...
        {
            "BusinessHoliday.json",
            "FeeModifierApplication.json",
            "PaymentMethod.json",
            "PromoCode.json",
            "Tier.json",
    ...
    private static async Task SeedJsonFile(AppDbContext context, string dataDir, string fileName)
    {
        ...
        var entityAssembly = AppDomain.CurrentDomain.GetAssemblies()
            .FirstOrDefault(a => a.GetTypes().Any(t => t.Namespace?.StartsWith("FeeCalculationEngine.Domain.Models") == true));
